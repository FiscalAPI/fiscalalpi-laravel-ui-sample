<div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-xs sm:gap-x-6 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
        @if($showSearch)
            <form action="{{ $searchAction }}" method="{{ $searchMethod }}" class="grid flex-1 grid-cols-1">
                <input name="search" placeholder="{{ $searchPlaceholder }}" aria-label="Search" class="col-start-1 row-start-1 block size-full bg-white pl-8 text-base text-gray-900 outline-hidden placeholder:text-gray-400 sm:text-sm/6" />
